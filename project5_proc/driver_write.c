...
ssize_t driver_writer(struct file *fp, const char  __user *ubuf, size_t size, loff_t * offset) 
{
	...
	printk(KERN_INFO "start Jiffies:%ld\n",start_time);
...
//	up(&f_ldev->sem);
	...
	printk(KERN_INFO "Data size in wr =%d\n",f_ldev->data_size);
	f_ldev->num_qset = nqset; 
	ssleep(1);	
	fp ->f_pos = bytes_written;
	end_time = get_jiffies_64();
	printk(KERN_INFO "End Jiffies:%ld\n",end_time);
	f_ldev ->time_spent = time_after(start_time,end_time);
	printk(KERN_INFO "Time Spent in writer =%d\n",f_ldev->time_spent);
	complete(&comp);	
